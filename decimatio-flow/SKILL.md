...
For server-side Apex called from Flow, see the companion skill `decimatio-apex`. For Lightning Web Components that embed or launch Flows, see `decimatio-lwc`.
...
Use `lightning/flowSupport` to embed a flow inside an LWC, or `standard__flow` PageReference to navigate to a screen flow (Summer '26 — see `decimatio-lwc` §6).
...
2. Route the variable to:
   ...
   - A logging subflow that creates an `Application_Log__c` record via Platform Event (cross-stack logging pattern — see `decimatio-apex` §11), or
