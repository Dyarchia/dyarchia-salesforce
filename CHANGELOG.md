...
- **Agent working files are no longer versioned.** `CLAUDE.md`, `.claude/skills/dya-skill-authoring/` and `docs/TODO.md` are working files for developing the library, not published content, and are now gitignored on every branch. The README's *Contributing* section states the skill contract inline instead of pointing at them, and the layout diagram drops the `CLAUDE.md` and `docs/` nodes. Side effect: `master` and `develop` hold an identical tree again, so syncing `master` is a plain fast-forward rather than a merge that has to re-exclude those paths.
