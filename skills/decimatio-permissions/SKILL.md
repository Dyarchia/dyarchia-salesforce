...
- **Apex v67 user mode** makes this model *enforced in code*: SOQL/DML default to `USER_MODE`, so the running user's object, field, and sharing access now governs what integration/controller code can read and write. The permission model is no longer "just UI" — it shapes code behaviour. See `decimatio-apex`.
- **Triggers always run in system mode** (all API versions) — they bypass CRUD/FLS/sharing regardless of v67.
- **"Any API Auth" permission** (new v67) gates legacy SOAP `login()` authentication — enforced by default in new orgs. Treat SOAP `login()` as end-of-life; migrate to OAuth + External Client Apps. See `decimatio-integration-auth`.
...
- Practical consequence: a too-narrow permission set or OWD can make user-mode code return fewer rows or throw; widening permissions to "fix" it also exposes data in reports and APIs. Fix the *model*, not the symptom. See `decimatio-apex`.
