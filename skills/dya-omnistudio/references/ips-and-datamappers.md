...
Load from `dya-omnistudio`. Server-side orchestration (IPs), data shaping (Data Mappers), and invoking them from code. Standard (`omnistudio`) and Managed Package (`vlocity_*`) differ in namespace — confirm the org's flavor.
...
- **REST / Connect API** — expose an IP as an API-callable endpoint for external systems (a high-performance, declarative alternative to hand-written Apex REST for transformation/orchestration at scale). Authenticate with OAuth (`dya-integration-auth`).
