...
Load from `dya-omnistudio`. The exact contract for calling custom Apex from FlexCards, OmniScripts, and Integration Procedures, for both flavors. It's Apex — `dya-apex` rules apply (bulk, `with sharing`, `WITH USER_MODE`, no SOQL/DML in loops).
