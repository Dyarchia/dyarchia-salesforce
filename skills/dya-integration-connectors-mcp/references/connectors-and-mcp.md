...
Default to **zero-copy** when the need is analytical/grounding and the data shouldn't be duplicated. See `dya-data360` for credits and mechanics.

## MCP as an Integration Surface

### Server types (recap; full detail in `dya-headless360`)
...
- **Security carries through** — every MCP call runs as the authenticated user with CRUD/FLS/sharing; auth is OAuth + PKCE via an ECA with `mcp_api`/`refresh_token` scopes (`dya-integration-auth`). MCP calls count against daily API limits.
