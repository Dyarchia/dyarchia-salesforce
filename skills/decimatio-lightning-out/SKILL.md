...
so everything in `decimatio-lwr` about runtime constraints applies; for the components
themselves, use `decimatio-lwc`.
...
- **Built on LWR**, not Aura — so **all `decimatio-lwr` runtime caveats apply**: verify
...
Whole experience could live on Salesforce          LWR site (decimatio-lwr-sites)
...
The embedded component runs on **LWR**, so `decimatio-lwr` fully applies: not all `lightning/*`
...
| Call an external endpoint from the LWC | CSP Trusted Site (it runs under LWS — see `decimatio-lwr`) |
...
| Embedding a whole experience that could be a site | Build an LWR site (`decimatio-lwr-sites`) |
...
| Assuming full LEX module/base-component availability | It is LWR — apply `decimatio-lwr` constraints |
...
5. **It is LWR underneath** — every `decimatio-lwr` constraint applies; small serialisable
