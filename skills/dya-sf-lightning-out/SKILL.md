...
site, another app. It runs on the **LWR runtime**, so everything in `dya-sf-lwr` applies; for the
components themselves, `dya-sf-lwc`. Follow every rule below.
...
exchange used here — see `dya-sf-integration-auth` if the host application authenticates any other way.
...
types will not resolve, and the session may be a limited identity. `dya-sf-lwr` carries the detail.
...
| Call an external endpoint from the LWC | CSP Trusted Site (it runs under LWS — see `dya-sf-lwr`) |
...
| Embedding a whole experience that could be a site | Build an LWR site (`dya-sf-lwr-sites`) |
