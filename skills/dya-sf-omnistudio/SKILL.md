...
builds on `dya-sf-apex` and `dya-sf-lwc`. Follow every rule below.
...
- Custom logic plugs in through **Apex Remote Actions**, and from API 67.0 that Apex defaults to
  ...
  `WITH USER_MODE`. See `dya-sf-permissions` for the model being enforced.
