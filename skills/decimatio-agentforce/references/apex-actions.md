...
Full implementations referenced from SKILL.md §3–§4. Load this when building or reviewing an agent action. Actions are how an agent *does* things; Apex actions are the deterministic backbone for logic the LLM must not improvise. They are Apex — the deep rules in `decimatio-apex` apply on top.
    ...
    public static List<Result> run(List<Request> requests) {
        ...
        for (Integer i = 0; i < srs.size(); i++) {
            ...
            if (srs[i].isSuccess()) {
                ...
                // Logger.error(...) via Platform Events — see decimatio-apex §11
            }
            results.add(res);
        }
        return results;
