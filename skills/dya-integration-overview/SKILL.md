...
- `dya-integration-inbound-apis` — standard APIs external systems call: REST (+composite), SOAP, Bulk 2.0, GraphQL, Connect/UI/Metadata/Tooling.
- `dya-integration-inbound-apex` — custom endpoints you expose: Apex REST (`@RestResource`), Apex SOAP, Sites/Experience Cloud.
- `dya-integration-outbound` — Salesforce calling out: Apex callouts + async, Flow HTTP Callout, External Services, Outbound Messages, Salesforce Connect, LWC→external.
- `dya-integration-events` — Platform Events, Change Data Capture, Pub/Sub API, webhook patterns.
- `dya-integration-auth` — OAuth flows, External Client Apps vs Connected Apps, Named/External Credentials, "Any API Auth", SOAP login() retirement.
- `dya-integration-connectors-mcp` — MuleSoft, Heroku, AppExchange, Hosted MCP servers / Headless 360 / Agent API.

This family sits **on top of** the core dyarchia skills and defers to them: async/governor detail → `dya-apex`; Flow mechanics → `dya-flow`; LWS/CSP → `dya-lwc`; agent actions → `dya-agentforce`; Data 360 ingestion → `dya-data360`; MCP/HXL → `dya-headless360`.
...
- **Hosted MCP servers are GA** — a first-class integration surface (`dya-integration-connectors-mcp`).
...
│    ├─ Standard data ops (CRUD/query/bulk) ... dya-integration-inbound-apis
...
- **Bulkification & governor limits** — ≤100 callouts per transaction, 6 MB/12 MB sync/async payloads; use Composite/Bulk to collapse API calls; never callout in a loop. Full detail in `dya-apex`.
...
LWC **cannot** call arbitrary Salesforce APIs from JavaScript (only LDS / `lightning/graphql`); for external APIs use an Apex proxy unless you have a specific reason to `fetch` directly. See `dya-integration-outbound` and `dya-lwc`.
...
- **Latency must be <15 min for analytics** → streaming; otherwise batch (see `dya-data360`).
