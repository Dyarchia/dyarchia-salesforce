...
so the right design choice becomes obvious. It is the skill other skills route to — `dya-sf-apex`,
`dya-sf-flow`, `dya-sf-lwc`, `dya-sf-integration-inbound-apex` and `dya-sf-agentforce` all enforce this model
without owning it. Authentication is a different concern and belongs to `dya-sf-integration-auth`.
...
system mode on every API version. See `references/shared/platform-deltas.md` and `dya-sf-apex`.
...
restrictive by accident; read what it actually grants. See `dya-sf-lwr-sites`.
