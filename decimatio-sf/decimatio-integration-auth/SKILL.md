...
- **Hosted MCP servers** authenticate via OAuth + **PKCE** with an ECA carrying `mcp_api` + `refresh_token` scopes (JWT-shaped tokens). See `decimatio-integration-connectors-mcp`.
...
- **Guest/Site access** — unauthenticated endpoints on Sites/Experience Cloud run as the guest user; lock the guest profile down (see `decimatio-integration-inbound-apex`).
