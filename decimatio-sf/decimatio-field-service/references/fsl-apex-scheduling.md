...
Load from `decimatio-field-service`. Real signatures and members for the `FSL` namespace scheduling classes, plus the scope-1 batch pattern. All `FSL.*` is managed-package code — **verify members in a v67 sandbox** (see SKILL.md §8) as they're version-dependent. Prerequisites: Field Service enabled + an FSL permission set on the running user.
