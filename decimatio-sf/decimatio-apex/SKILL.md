...
- `@IsTest(testFor='ApexClass:AccountService,ApexTrigger:AccountTrigger')` — forces the test to run when the listed components are part of the deployment.

Both annotations only take effect with `sf project deploy start --test-level RunRelevantTests`. Until GA, production-critical tests should still be verified by a broader test level.
