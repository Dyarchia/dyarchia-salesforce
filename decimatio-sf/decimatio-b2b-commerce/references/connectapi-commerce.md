...
Load from `decimatio-b2b-commerce`. The `ConnectApi.CommerceCart` Apex API, the buyer-group/entitlement model that governs visibility, and Storefront LWC. Confirm exact method overloads against the Apex Reference for your API version.
...
B2B/D2C **LWR** stores support **Storefront APIs** to build custom LWC (headers, footers, cart widgets, PLP/PDP components). Rules (`decimatio-lwc`):
...
For an external front-end, call the **Commerce Connect REST** APIs (address management, carts, checkouts, orders, products, promotions, taxes, wishlists, search). Authenticate per `decimatio-integration-auth`; the same entitlement rules apply.
