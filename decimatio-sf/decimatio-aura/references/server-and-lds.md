...
Full implementations referenced from SKILL.md §4–§5. Load this when accessing data from an Aura component on Summer '26 (API v67.0). Evaluate LDS first; reach for `@AuraEnabled` Apex only for genuine server-side logic. Aura controllers are Apex — deep server rules live in `decimatio-apex`.
