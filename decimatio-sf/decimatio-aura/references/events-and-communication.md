...
The `onMessage` attribute subscribes declaratively; no manual subscribe/unsubscribe needed. `scope="APPLICATION"` receives messages regardless of where the component sits (e.g. utility bar). For the LWC side of the same channel see `decimatio-lwc`; for the Visualforce side (`sforce.one.publish/subscribe`) see `decimatio-visualforce`.
