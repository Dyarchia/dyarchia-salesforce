...
`decimatio-lwr`; for the components themselves, use `decimatio-lwc`.
...
- Custom LWCs on the site run on the **LWR runtime** — all of `decimatio-lwr` applies (module
...
- Every guest-facing component must render a clean **empty / access-denied** state (enforced
  component-side in `decimatio-lwr`).
...
- Keep pages lean (`decimatio-lwr` §7); Core Web Vitals affect ranking.
...
| Custom UI beyond standard components | Custom LWC (apply `decimatio-lwr`) |
...
| `lightning-file-upload` for site uploads | Supported mechanism; mind the guest (`decimatio-lwr`) |
