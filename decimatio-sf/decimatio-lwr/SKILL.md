...
difference. For LWC authoring itself (templates, LDS, GraphQL, state), use `decimatio-lwc`;
for building Experience sites on LWR, use `decimatio-lwr-sites`; for embedding LWCs in
non-Salesforce apps, use `decimatio-lightning-out`.
...
- **v67 cross-cutting:** LWS now **blocks `data:` URIs** — build client-side downloads with
  `blob:` URLs (see `decimatio-lwc`). `WITH SECURITY_ENFORCED` no longer compiles in Apex
...
Mental model: **`decimatio-lwc` teaches the component; this skill teaches the runtime it lands
...
(Site-level guest hardening lives in `decimatio-lwr-sites`; this is the component-side
discipline.)
