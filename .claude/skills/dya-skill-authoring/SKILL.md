---
name: dya-skill-authoring
description: Authoring procedure for the dyarchia-salesforce skill library — adding a skill, editing an existing one, splitting an oversized SKILL.md, rebuilding and validating .skill bundles, and updating README and CHANGELOG so they never drift. Load only when the user explicitly invokes this skill by name (`dya-skill-authoring`); do NOT auto-trigger on generic skill, documentation, or Salesforce questions.
---

# dyarchia-salesforce — Skill Authoring
...
Every skill exists twice: as a source folder under `skills/dya-<name>/` and as a committed
bundle `dist/dya-<name>.skill`. **Drift between them is the failure mode of this repo.** The
validator compares them file by file, by content hash. Never edit a bundle directly; always
regenerate it from source.

There is now a third generated artifact with the same discipline. Platform fundamentals — governor
limits, the access model, SOQL selectivity, API version semantics, the org model, the release
deltas — live once in `references-shared/` at the repo root. A skill declares the fragments it needs
in `skills/dya-<name>/shared-refs.txt`, one name per line, and `scripts/sync-shared-refs` copies
them into `skills/dya-<name>/references/shared/`. Those copies are **committed**, so every `.skill`
bundle stays self-contained and the builder needs no special case.

**Never edit a file under `references/shared/`.** Edit the canon and re-sync; the validator compares
every copy against its canon by hash and fails on a difference. A fact that belongs to more than one
skill belongs in the canon, not copy-pasted — that duplication is what made the last platform
version bump a sweep across 267 occurrences in 58 files.
...
2. Create `skills/dya-<name>/SKILL.md` with the frontmatter contract:
   ...
   - `description` stating domain and platform version, then the surface covered, then verbatim:
     `Load only when the user explicitly invokes this skill by name (\`dya-<name>\`); do NOT
...
6. Declare any shared fragments the skill needs in `skills/dya-<name>/shared-refs.txt`, cite each
   one by filename from `SKILL.md` (an uncited reference is a validation error), and sync:
...
2. Sync, then rebuild that skill's bundle — step 6 above. A source edit without a rebuild is the
   drift the validator exists to catch.
...
README badge agrees with the catalogue    The two halves of the README disagree
...
`dya-b2c-commerce` (Demandware lineage, no core API version) and `dya-sf-cli` (the CLI versions
weekly, independently of the platform). Like `nonSkillTokens`, that allowlist is hardcoded in
**both** twins.

All 25 skills are currently under the size ceiling, so a clean tree validates with **zero errors and
zero warnings**. A new warning is something to fix in the same commit, not debt to carry.
